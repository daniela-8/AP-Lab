...
from operations.borrow_service import BorrowService
...
class UI:
    def __init__(self, bookRepository, customerRepository, borrowRepository):
        self.bookService = BookService(bookRepository)
        self.customerService = CustomerService(customerRepository)
        self.borrowService = BorrowService(borrowRepository, bookRepository, customerRepository)
    ...
    def display_menu(self):
        ...
        print("13. Borrow book")
        print("14. Return book")
        print("15. Display borrowings")
        print("0. Exit")
    ...
    def run(self):
        ...
        self.bookService.add_book(3, "Punguța cu doi bani", "A classic tale", "Ion Creangă")
        ...
        while True:
            ...
            try:
                ...
                if command == 1:
                    ...
                    borrowing = self.borrowService.borrow_book(customer_id, book_id)
                    print(f"Book borrowed successfully! Borrowing ID: {borrowing.borrowing_id}")
                elif command == 14:
                    borrowing_id = int(input("Enter borrowing id to return: "))
                    self.borrowService.return_book(borrowing_id)
                    print("Book returned successfully!")
                elif command == 15:
                    borrowings = self.borrowService.get_all_borrowings()
